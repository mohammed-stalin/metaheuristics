...
def f19_composite_griewank_rosenbrock(x):
    ...
    return np.sum(100 * (z[1:] - z[:-1]**2 / 4000 - np.cos(100 * (z[1:] - z[:-1]**2))) + 1) + 10 - 360
...
def f23_katsuura(x):
    z = generate_rotation_matrix(len(x)) @ (x - 0.5)
    return -10 / len(x)**2 * np.prod(1 + np.arange(1, len(x)+1) * np.sum([np.abs(2j * z - np.round(2j * z)) / 2*j for j in range(1, 33)])) - 360
